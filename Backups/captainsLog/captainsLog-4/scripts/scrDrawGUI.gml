...
for(i = 0; i < array_length_1d(epitaphs); i++) {
    ...
    if(epitaph.target != noone) {
        ...
        draw_sprite_ext(object_get_sprite(epitaph.toRevive), -1, epitaph.target.x + (epitaph.target.sprite_width / 2), epitaph.target.y + (epitaph.target.sprite_width / 2), 1, 1, 0, c_white, 1);
    }
}
